...
object WhisperService {
    ...
    suspend fun transcribeWav(wavFile: File): String = withContext(Dispatchers.Default) {
        ...
        Log.d(LOG_TAG, "WAV decoded: samples=${floats.size} sr=16000 durMs=${floats.size / 16} (decode=${durDecode}ms)")
    ...
    suspend fun transcribeWavWithTimestamps(wavFile: File): List<WhisperSegment> =
        withContext(Dispatchers.Default) {
            ...
            Log.d(LOG_TAG, "WAV decoded: samples=${floats.size} sr=16000 durMs=${floats.size / 16}")

            try {
                val t0 = System.currentTimeMillis()
                val segments = c.transcribeDataWithTime(floats)
                val t1 = System.currentTimeMillis()
                Log.d(LOG_TAG, "Whisper transcribeDataWithTime() done in ${t1 - t0} ms (segments=${segments.size})")
                segments
            } catch (e: ExecutionException) {
                throw (e.cause ?: e)
            }
    ...
    private fun ensureCtx(): WhisperContext =
        ctx ?: error("Whisper model not loaded yet. Call loadModel() once at app startup.")
}
