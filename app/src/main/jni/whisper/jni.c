...
static size_t inputStreamRead(void *ctx, void *output, size_t read_size) {
    ...
    jint size_to_copy = (jint) (read_size < (size_t) avail_size ? read_size : (size_t) avail_size);
    ...
    if (size_to_copy != (jint) read_size || size_to_copy != n_read) {
        LOGI("Insufficient Read: Req=%zu, ToCopy=%d, Read=%d", read_size, size_to_copy, n_read);
    }
...
static void inputStreamClose(void * ctx) { UNUSED(ctx); }
...
Java_com_whispercpp_java_whisper_WhisperLib_initContextFromInputStream(
    ...
    (void) loader.eof(loader.context);
...
JNIEXPORT jstring JNICALL
Java_com_whispercpp_java_whisper_WhisperLib_benchMemcpy(
...
JNIEXPORT jstring JNICALL
Java_com_whispercpp_java_whisper_WhisperLib_benchGgmlMulMat(
        JNIEnv *env, jobject thiz, jint n_threads) {
    UNUSED(thiz);
